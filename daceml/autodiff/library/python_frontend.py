...
from dace import SDFG, SDFGState, config, data, hooks
...
def backward(pv: newast.ProgramVisitor,
    ...
    # determine what grdaiients to compute
...
def expand_backward_pass_nodes(sdfg: SDFG):
    """
    Expands all BackwardPass nodes in the SDFG.
    This is registered as a hook to be called before SDFG compilation.
    """
    # expand_nodes(sdfg, lambda n: isinstance(n, BackwardPass))
    expand_nodes(sdfg, lambda n: True)


# Register the hook to expand BackwardPass nodes before SDFG compilation
hooks.register_sdfg_call_hook(before_hook=expand_backward_pass_nodes)
